...
var populate = function(body) {
  var query = {
    query: {
      bool: {
        must: [{ match: { searchable: body.term } }],
        should: [
          
          { match: { year: { query: body.term, boost: 10 } } }
        ],
        filter: [],
        must_not: []
      }
...
var search = function(body) {
  return new Promise((resolve, reject) => {
    ...
    query.aggs = aggs;
    if (body.style) {
      query.query.bool.must.push({ match: { "style.keyword": body.style } });
    }
    if (body.genre) {
      query.query.bool.must.push({ match: { "genre.keyword": body.genre } });
    }
    if (body.dates) {
